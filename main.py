...
def remove_doc_from_shelf(doc_number, directories_dict):
    for directory_number, directory_docs_list in directories_dict.items():
        if doc_number in directory_docs_list:
            directory_docs_list.remove(doc_number)
            break


def add_new_shelf(directories_dict, shelf_number=''):
    if not shelf_number:
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            shelf_number, added = add_new_shelf(directories)
            if added:
